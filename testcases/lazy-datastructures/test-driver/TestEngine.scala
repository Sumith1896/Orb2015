...
//          /if(i%100000 == 0) println("Enqueue..")         
  ...
  def numericalRepTest() {
    ...
    var lazynum = LazyNumericalRep.emptyNum        
    var totalTime1 = 0L
    var totalTime2 = 0L
    println(s"Testing amortized emphemeral behavior on $ops operations...")
    for (i <- 0 until ops) {
      ...
      lazynum = timed { LazyNumericalRep.incAndPay(lazynum) } { totalTime2 += _ }
      val d1 = BigNums.firstDigit(bignum)
      val d2 = LazyNumericalRep.firstDigit(lazynum)      
    ...
    bignum = BigNums.Nil()
    lazynum = LazyNumericalRep.emptyNum    
    for (i <- 0 until length) {
      bignum = BigNums.increment(bignum)
      lazynum = LazyNumericalRep.incAndPay(lazynum)
    }
    ...
    timed { LazyNumericalRep.incAndPay(lazynum) } { t => println(s"Time for one lazy increment in the worst case: ${t/1000.0}s") }
  }
